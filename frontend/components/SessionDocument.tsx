...
import SavedLiveNotesRail from "./SavedLiveNotesRail";
...
export default function SessionDocument({
  ...
  const liveEntries =
    session.live_notes_history?.map((entry, index) => ({
      id: `${session.id}-${entry.timestamp}-${index}`,
      ts: entry.timestamp,
      notes: entry.notes as {
        nowTopic?: string;
        keyPoints?: string[];
  ...
  return (
    <article className={`session-document${printMode ? " session-document--print" : ""}`}>
      ...
      <div className="session-document-layout">
        <div className="session-document-main">
          ...
          {includeStudentNotes && session.student_notes_text && (
            <section className="session-document-block session-document-side-block">
              <h2>Student Notes</h2>
        ...
        {includeTimeline && liveEntries.length > 0 && (
          <aside className="session-document-rail">
            <SavedLiveNotesRail entries={liveEntries} title="Live Notes" />
          </aside>
        )}
      </div>

      <footer className="session-document-footer">
        <span>Generated by LiveLecture</span>
        <span>{session.course_code ?? "Lecture session"}</span>
